sap.ui.define(["QuickStartApplication/controller/BaseController"], function (BaseController) {
	"use strict";
	return BaseController.extend("QuickStartApplication.controller.inicioHojaDeGastos", {
		...
		onPressGoToTasks : function(oEvent) {
		    // establezco el nombre del proyecto seleccionado a la pagina
	        var context = oEvent.oSource.getBindingContext("projects");
        	var nombre = context.getModel().getProperty("Nombre", context);
			this.byId("masterTasks").setTitle(nombre);
			
			// paso a listTasks las tareas del proyecto seleccionado
			var listTasks = this.getView().byId("listTasks");
			var itemListTasks = this.getView().byId("itemListTasks");
			listTasks.setModel(this.getView().getModel("projects"));
			listTasks.bindItems(context.getPath()+"/Tareas", itemListTasks);
			
			this.getSplitAppObj().toMaster(this.createId("masterTasks"));
		},
